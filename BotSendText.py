...
logging.basicConfig(filename='loggi.txt', filemode='a', format='%(asctime)s - %(message)s', datefmt='%d-%b-%y %H:%M:%S',
                    level=logging.INFO)
...
def datanotfound(message):
    ...
    if len(diction1) == 2:
        ...
        bot.send_message(message.chat.id, 'Такой команды нет(', reply_markup=keyboards.keyboard1)
...
def for_user_text(message):
    ...
    if message.text == emoji.emojize('Найти серию по фразе :speech_balloon:', use_aliases=True):
        ...
        bot.send_message(message.chat.id,
                         'Первый сезон задрал планку экшена на безумно высокий уровень, потому финал '
        ...
        bot.send_photo(message.chat.id, 'AgACAgIAAxkBAAII4l'
                                        ...
                                        'AA3kAAwRCAgABGwQ')
        bot.send_message(message.chat.id,
                         'Но самый лучший на сегодняшний день поединок в «Ванпанчмене» случился в '
        ...
        bot.send_message(message.chat.id,
                         'Морской Царь оказался первой серьезной угрозой, появившейся в аниме. Также '
        ...
        bot.send_message(message.chat.id,
                         "Наглядный пример находчивости и необычного чувства юмора ONE. По своей "
        ...
        bot.send_photo(message.chat.id, 'AgACAgIAAxkBAAIJEF'
                                        ...
                                        '-lAQABGwQ')
        bot.send_message(message.chat.id,
                         "Важнейшая битва для создания максимально притягательного образа главного "
        ...
        bot.send_message(message.chat.id, 'Приветствую тебя у нас', reply_markup=keyboards.keyboard1)
    elif message.text.lower() == 'пока':
        bot.send_message(message.chat.id, 'Возвращайся скорее', reply_markup=keyboards.keyboard1)
    elif message.text == emoji.emojize('Смотреть серии :clapper:', use_aliases=True):
        bot.send_message(message.chat.id, 'Выбери предпочтения', reply_markup=keyboards.keyboard2)
    elif message.text.lower() == 'что такое ova?':  # c-eng
        logging.info(f'user- {message.from_user.id}, enter- {message.text}')
        bot.send_message(message.chat.id,
                         'OVA-это спецвыпуски, которые созданы в качетсве дополненительных серий (не '
                         'относятся к основному сюжету аниме).',
                         reply_markup=keyboards.keyboard1)
    elif message.text.lower() == 'нaзад':  # a-eng
        bot.send_message(message.chat.id, 'Вернулись назад', reply_markup=keyboards.keyboard1)
    elif message.text.lower() == 'нaзaд':  # aa-eng
        bot.send_message(message.chat.id, 'Вернулись назад', reply_markup=keyboards.keyboard2)
        ...
        bot.send_message(message.chat.id, 'Другое', reply_markup=keyboards.keyboard11)
    elif message.text.lower() == 'назaд':  # second_a-eng
        bot.send_message(message.chat.id, 'Вернулись в главное меню', reply_markup=keyboards.keyboard1)
    elif message.text.lower() == 'веpнуться':  # p-eng
        bot.send_message(message.chat.id, 'Вернулись в меню', reply_markup=keyboards.keyboard3)
    elif message.text.lower() == 'веpнyться':  # yp-eng
        bot.send_message(message.chat.id, 'Вернулись в меню', reply_markup=keyboards.keyboard3)
    elif message.text.lower() == 'вернyться':  # y-eng
        bot.send_message(message.chat.id, 'Вернулись в меню', reply_markup=keyboards.keyboard8)
    elif message.text.lower() == 'вepнyться':  # eyp-eng
        bot.send_message(message.chat.id, 'Вернулись в меню', reply_markup=keyboards.keyboard8)
    elif message.text.lower() == 'вeрнyться':  # ey-eng
        bot.send_message(message.chat.id, 'Вернулись в меню', reply_markup=keyboards.keyboardOV)
    elif message.text.lower() == 'веpнyтьcя':  # ypc-eng
        bot.send_message(message.chat.id, 'Вернулись в меню', reply_markup=keyboards.keyboardOV)
    elif message.text.lower() == '1 сезон':
        bot.send_message(message.chat.id, 'Смотри 1 сезон', reply_markup=keyboards.keyboard3)
    elif message.text.lower() == '1-6 cерии':  # c-eng
        bot.send_message(message.chat.id, 'Смотри 1-6 серии', reply_markup=keyboards.keyboard4)
    elif message.text.lower() == '7-12 cерии':  # c-eng
        bot.send_message(message.chat.id, 'Смотри 7-12 серии', reply_markup=keyboards.keyboard5)
        ...
        bot.send_message(message.chat.id, 'Смотри 2 сезон', reply_markup=keyboards.keyboard8)
    elif message.text.lower() == '1-6 ceрии':  # ce-eng
        bot.send_message(message.chat.id, 'Смотри 1-6 серии', reply_markup=keyboards.keyboard6)
    elif message.text.lower() == '7-12 ceрии':  # ce-eng
        bot.send_message(message.chat.id, 'Смотри 7-12 серии', reply_markup=keyboards.keyboard7)
        ...
        bot.send_message(message.chat.id, 'Смотри OVA', reply_markup=keyboards.keyboardOV)
    elif message.text.lower() == '1 ceзон ova':  # ce-eng
        bot.send_message(message.chat.id, 'Смотри 1 сезон', reply_markup=keyboards.keyboardOV1)
    elif message.text.lower() == '2 ceзон ova':  # ce-eng
        bot.send_message(message.chat.id, 'Смотри 2 сезон', reply_markup=keyboards.keyboardOV2)
        ...
        bot.send_photo(message.chat.id,
                       ...
                       '-AgABGwQ',
                       'Милая маска/Прекрасно замаскированная Милая маска: 1 место. Маскот Ассоциации '
        ...
        bot.send_photo(message.chat.id,
                       'AgACAgIAAxkBAAILEl'
                       '-EW_zknsjzKMleP0vcGYxobOdtAAIFsDEbDEkgSHSeGEDHt1wHuxJwly4AAwEAAwIAA20AAzS8AQABGwQ',
                       "Бэнг")
        bot.send_message(message.chat.id,
...
req = {'уровни угроз': SendThreats, 'демонический': SendThreats, 'волчий': SendThreats, 'драконий': SendThreats,
       ...
       'сверхлитой темноблеск': HeroSverx, 'свинобог': HeroPig, 'технорыцарь': HeroKnight,
       'зомбимен': HeroZombie, 'кинг': HeroKing, 'бофой': HeroBofoi}
...
text = {'уровни угроз': 'Ассоциацией Героев было создано разделение монстров по уровням опасности.\n\n'
        ...
        'демонический': 'Демонический/Дьявольский(в зависимости от перевода) — угроза целому городу.',
        'волчий': 'Волчий — угроза нескольким людям.', 'драконий': 'Драконий — угроза нескольким городам.',
        'тигриный': 'Тигриный — угроза большому количеству человек.',
        'божественный': 'Божественный — угроза всему человечеству.'}
